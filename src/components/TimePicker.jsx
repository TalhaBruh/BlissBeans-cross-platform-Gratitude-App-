...
const TimePicker = ({ onTimeChange }) => {
  ...
  const renderHour = ({ item }) => (
    <TouchableOpacity>
      <Text style={{ fontSize: 50, color: item === selectedHour ? 'yellow' : 'gray' }}>{item}</Text>
    </TouchableOpacity>
  );

  const renderMinute = ({ item }) => (
    <TouchableOpacity>
      <Text style={{ fontSize: 50, color: item === selectedMinute ? 'yellow' : 'gray' }}>{item}</Text>
    </TouchableOpacity>
  );

  const renderAmPm = ({ item }) => (
    <TouchableOpacity>
      <Text style={{ fontSize: 50, color: item === selectedAmPm ? 'yellow' : 'gray' }}>{item}</Text>
    </TouchableOpacity>
  );
